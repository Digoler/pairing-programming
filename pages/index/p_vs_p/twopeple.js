Page({
    ...
    shakeClick: function () { 
      ...
      if(!this.global.flag && this.data.allow_click=="auto"){
        this.setData({['resp.wrap_1']:"true", allow_click: "auto"});
      }else if(this.global.flag && this.data.allow_click=="auto"){
        this.setData({['resp.wrap']:"true", allow_click: "auto"});
      }
      this.setData({
        dice: this.data.dice,
        allow_click: "none"
    ...
    locationClick:function(e){
      if(this.global.flag && this.global.setdice){
        this.setData({['resp.wrap']:"none", allow_click: "auto"});
      }else{
        this.setData({['resp.wrap_1']:"none", allow_click: "auto"});
      }
      this.global.flag = !this.global.flag;
      this.global.setdice = !this.global.setdice;
      e = e || Event;
      let tap = e["currentTarget"]["id"];
      tap = tap.replace('-','_');
      switch(tap){
            case "box_1":
            this.setData(
              {['incident.box_1'] : 0,['showdata.box_1'] : this.data.num1,box_1 : "true",['resp.wrap']:"none",
              'player_a[0]':this.data.num1+1}
            );
            ...
            case "box_5":
            this.setData(
              {['incident.box_5'] : 0,['showdata.box_5'] : this.data.num1,box_5 : "true",['resp.wrap']:"none",'player_a[4]':this.data.num1+1}
            );
            // 消除玩家B的骰子，并计算出B被消除后的分数
            ...
            case "box_6":
            this.setData(
              {['incident.box_6'] : 0,['showdata.box_6'] : this.data.num1,box_6 : "true",['resp.wrap']:"none",'player_a[5]':this.data.num1+1}
            );
            ...
            case "box_9":
            this.setData(
              {['incident.box_9'] : 0,['showdata.box_9'] : this.data.num1,box_9 : "true",['resp.wrap']:"none",'player_a[8]':this.data.num1+1}
            );
            ...
            case "box_10":
              ...
              for(var index=position; index<9; index+=3){
                  if(this.data.player_a[index] == this.data.num1+1)
                  {
                    ...
                    if(temp_box == "box_1"){
                      this.setData({player_a:temp_list, box_1:"none",['incident.box_1'] : "locationClick"});
                    }else if(temp_box == "box_4"){
                      this.setData({player_a:temp_list, box_4:"none",['incident.box_4'] : "locationClick"});
                    }else if(temp_box == "box_7"){
                      this.setData({player_a:temp_list, box_7:"none",['incident.box_7'] : "locationClick"});
                    }
                  }
              }
              this.compute_score_b(this.data.player_b)
            ...
            this.gameover();
              ...
              for(var index=position; index<9; index+=3){
                  if(this.data.player_a[index] == this.data.num1+1)
                  {
                    ...
                    let temp_box = "box_" + `${index+1}`;
                    if(temp_box == "box_2"){
                      this.setData({player_a:temp_list, box_2:"none",['incident.box_2'] : "locationClick"});
                    }else if(temp_box == "box_5"){
                      this.setData({player_a:temp_list, box_5:"none",['incident.box_5'] : "locationClick"});
                    }else if(temp_box == "box_8"){
                      this.setData({player_a:temp_list, box_8:"none",['incident.box_8'] : "locationClick"});
                    }
                  }
              }
              this.compute_score_b(this.data.player_b)
              ...
              for(var index=position; index<9; index+=3){
                  if(this.data.player_a[index] == this.data.num1+1)
                  {
                    ...
                    let temp_box = "box_" + `${index+1}`;
                    if(temp_box == "box_3"){
                      this.setData({player_a:temp_list, box_3:"none",['incident.box_3'] : "locationClick"});
                    }else if(temp_box == "box_6"){
                      this.setData({player_a:temp_list, box_6:"none",['incident.box_6'] : "locationClick"});
                    }else if(temp_box == "box_9"){
                      this.setData({player_a:temp_list, box_9:"none",['incident.box_9'] : "locationClick"});
                    }
                  }
              }
              this.compute_score_b(this.data.player_b)
    ...
    //回合提示
    round_notice:function(){
      if(this.global.setdice){
        this.setData({notice:"A's Round"})
      }else{
        this.setData({notice:"B's Round"})
      }
      wx.showToast({
        title: this.data.notice,
        image: '/images/notice/提示.png',
        duration: 1000,
        mask:true
      })
    },
    ...
    //判断游戏是否结束
    gameover:function(){
      var over_a = 1;//为1则游戏结束
      //遍历是否还有未放置的位置
      for(var index=0;index<9;index++){
        if(this.data.player_a[index] == 0){
          over_a = over_a - 1;//有位置还没有被放置，over_a置为0
          break;
        }
      }
      //游戏结束，选出winner
      if(over_a){
        ...
        this.data.score_a > this.data.score_b?(this.setData({winner:"A"})):this.setData({winner:"B"})
      }
      var over_b = 1;
      for(var index=0;index<9;index++){
      ...
      if(over_b){
        ...
        if(this.data.score_a > this.data.score_b){
          this.setData({winner:"A"})
        }else{
          this.setData({winner:"B"})
        }
      }
